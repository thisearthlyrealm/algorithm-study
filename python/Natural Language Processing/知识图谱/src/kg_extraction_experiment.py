...
# 一、关键参数配置
# =========================
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "02_实验结果文件"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 中文字体路径：Linux 常见环境使用 NotoSansCJK；Windows 可改为 C:/Windows/Fonts/simhei.ttf
FONT_CANDIDATES = [
...
if FONT_PATH is None:
    raise FileNotFoundError("未找到可用中文字体，请在 FONT_CANDIDATES 中配置本机中文字体路径。")
...
DOMAIN_TERMS = sorted(set(DOMAIN_TERMS), key=len, reverse=True)
...
def extract_entities(text, entity_types):
    """通过领域词典最长匹配识别实体，并记录出现次数和类型。

    这里不直接使用 text.count，是为了避免“实体识别”被重复统计为“实体”、
    “未标注数据”被误统计为“标注数据”等子串误匹配问题。
    """
...
def extract_relations(text, entity_types):
    ...
    该函数先在句子内定位实体，再根据“是……分支/子领域、通过、使用、用于、包括、由……构成、表示、描述”等
    触发词确定关系类型。与简单共现相比，这种方法能减少“相邻实体误连”的问题。
    """
    triples = []
    seen = set()
    ...
    def entity_occurrences(sentence):
        """返回句子中所有实体出现位置，按位置排序；同一实体多次出现时全部保留。"""
        occ = []
        for e in entities:
            start = 0
            while True:
    ...
    for sentence in split_sentences(text):
        ...
        for trigger in ["应用于", "用于"]:
            ...
            if trigger in sentence:
                ...
                if before and after:
                    if trigger == "应用于":
                        # “知识图谱可应用于智能问答、语义检索和推荐系统”属于并列应用对象
                        for tail in after:
                            add(before[-1], "应用于", tail, sentence)
                    else:
                        # “卷积神经网络常用于计算机视觉中的图像分类……”中，先连接到应用方向，再连接具体任务
        ...
        if "包括" in sentence:
            ...
            if before and after:
                # 对“常见任务包括……”这类句子，主语应为学习范式，而不是离“包括”最近的数据实体
...
def main():
    ...
    ent_df.to_csv(OUTPUT_DIR / "entities.csv", index=False, encoding="utf-8-sig")
